...
proj <- loadArchRProject("X041-AP0C1W6_ArchRProject/")
...
saveRDS(enrichMotifs,
        "X041-AP0C1W6_enrichMotifs.rds")

enrichMotifs <- readRDS("X041-AP0C1W6_enrichMotifs.rds")
...
motifs_df <- assay(enrichMotifs) %>%
  ...
  mutate(well_label = "Perm: No Neutrophils",
         well_id = "X041-AP0C1W6") %>%
...
pw_df <- map2_dfr(1:ncol(pw_types),
                  ...
                  function(pw_col, pw_res) {
                    ...
                    if(nrow(res) > 0) {
                      res$well_label <- "Perm: No Neutrophils"
                      res$well_id <- "X041-AP0C1W6"
...
fwrite(pw_df,
       "X041-AP0C1W6_pw_peak_df.csv.gz")
