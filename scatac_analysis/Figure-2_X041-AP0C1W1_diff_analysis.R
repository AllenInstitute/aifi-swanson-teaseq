...
proj <- loadArchRProject("X041-AP0C1W1_ArchRProject/")
...
saveRDS(enrichMotifs,
        "X041-AP0C1W1_enrichMotifs.rds")

enrichMotifs <- readRDS("X041-AP0C1W1_enrichMotifs.rds")
...
motifs_df <- assay(enrichMotifs) %>%
  ...
  mutate(well_label = "Nuclei: Unsorted",
         well_id = "X041-AP0C1W1") %>%
...
pw_df <- map2_dfr(1:ncol(pw_types),
                  ...
                  function(pw_col, pw_res) {
                    ...
                    if(nrow(res) > 0) {
                      res$well_label <- "Nuclei: Unsorted"
                      res$well_id <- "X041-AP0C1W1"
...
fwrite(pw_df,
       "X041-AP0C1W1_pw_peak_df.csv.gz")
