...
while getopts "b:i:g:j:o:s:t:w:" opt; do
  case $opt in
...
echo $(stm "PARAM -b BedTools Location     : ${bedtools_loc}")
echo $(stm "PARAM -i Input fragments.tsv.gz: ${in_fragments}")
echo $(stm "PARAM -g Genome (hg19 or hg38) : ${genome}")
echo $(stm "PARAM -j N Parallel Jobs       : ${num_jobs}")
echo $(stm "PARAM -o Output Directory      : ${output_dir}")
echo $(stm "PARAM -s Input singlecell.csv  : ${in_singlecell}")
echo $(stm "PARAM -t Temporary Directory   : ${temp_dir}")
echo $(stm "PARAM -w Well ID               : ${well_id}")
...
if [ -f ${in_singlecell} ]; then
  echo $(stm "Generating whitelist from $in_singlecell")

  cat ${in_singlecell} \
  | awk -F',' -v OFS='\t' \
    -v a=${temp_dir}/whitelist/ \
    -v c=.tsv \
    '{ if($8 > 500 && NR > 2) { b=substr($1,0,4); print $1 > a b c } }'
else
  in_singlecell=$( echo ${in_singlecell} | sed 's/singlecell.csv/barcode_counts.tsv/g' )
  ...
  | awk -F'\t' -v OFS='\t' \
    -v a=${temp_dir}/whitelist/ \
    -v c=.tsv \
    '{ if($2 > 500 && NR > 2) { b=substr($1,0,4); print $1 > a b c } }'
fi
