...
if(is.null(params$in_pre)) {
  ...
  in_tenx <- system.file("testdata/outs", package = "ATAComb")
  in_sum <- system.file("testdata/outs", package = "ATAComb")
...
if(!dir.exists(in_tenx)) {
  ...
  in_pbm <- file.path(in_tenx, "per_barcode_metrics.csv")
  if(!file.exists(in_pbm)) {
    stm(paste0("ERROR: Cannot find 10x per_barcode_metrics.csv:", in_pbm))
    stop()
  }
  ...
  if(!file.exists(in_frag)) {
    stm(paste0("ERROR: Cannot find 10x atac_fragments.tsv.gz:", in_tenx))
    stop()
  }
}
...
stm(paste0("SampleSheet WellID: ", in_well, "; SampleID: ", in_sample))
...
```{r}
stm(paste0("Reading 10x metadata from ", in_pbm))

if(grepl("per_barcode_metrics.csv", in_pbm)) {
  tenx_meta <- fread(in_pbm)
  tenx_meta <- tenx_meta[, list(barcode, atac_raw_reads, atac_dup_reads, atac_mitochondrial_reads, atac_fragments)]
  names(tenx_meta) <- c("original_barcodes","n_fragments","n_duplicate","n_mito","n_unique")
} else if(grepl("barcode_counts.tsv", in_pbm)) {
...
stm(paste("Before filtering:",nrow(meta),"barcodes"))
filtered_meta <- meta
...
stm(paste("After filtering:",nrow(filtered_meta),"barcodes"))
```
...
```{r}
stm("ArchR: Scoring and filtering doublets")
...
unfiltered_archr_names <- getCellNames(proj)
...
if(min(doublet_scores[[1]]$doubletEnrich) < 1) {
  ...
  # Get filtered names and original barcodes
  archr_meta <- getCellColData(proj)
  filtered_archr_names <- rownames(archr_meta)
  filtered_archr_barcodes <- sub(".+#","",filtered_archr_names)
  archr_meta <- archr_meta[filtered_archr_names,]
  
  filter_doublets <- TRUE
} else {
  stm("WARNING: DOUBLET SCORING FAILED - retaining all cells.")
  
  # Don't filter names and original barcodes
  archr_meta <- getCellColData(proj)
  filtered_archr_names <- rownames(archr_meta)
  filtered_archr_barcodes <- sub(".+#","",filtered_archr_names)
  archr_meta <- archr_meta[filtered_archr_names,]
...
n_pre_doublet_filter <- nrow(filtered_meta)

stm(paste0("ArchR: Total barcodes = ",n_pre_doublet_filter))

meta <- meta %>%
  dplyr::mutate(singlet = original_barcodes %in% filtered_archr_barcodes)

filtered_meta <- filtered_meta %>%
  dplyr::filter(original_barcodes %in% filtered_archr_barcodes)

n_post_doublet_filter <- nrow(filtered_meta)
stm(paste0("ArchR: Singlet barcodes = ",n_post_doublet_filter))

n_doublets <-  n_pre_doublet_filter - n_post_doublet_filter
stm(paste0("ArchR: Doublet barcodes = ",n_doublets))

# Match order to archr file
filtered_meta <- filtered_meta[match(filtered_meta$original_barcodes, filtered_archr_barcodes),]
...
```{r}
if(filter_doublets) {
  stm("ArchR: Subsetting to remove doublets")
  proj <- subsetArchRProject(proj,
                             cells = filtered_archr_names)
}
...
Resolve metadata discrepencies
```{r}
...
h5_MetaNames <- h5_MetaNames[h5_MetaNames$dim != 1,]
h5_MetaNames <- h5_MetaNames[h5_MetaNames$dim != length(filtered_archr_names),]

if(nrow(h5_MetaNames) > 0) {
  h5_CellNames <- h5read(out_arrow, "/Metadata/CellNames")
  h5_keepLogic <- h5_CellNames %in% filtered_archr_barcodes
  
  for(i in 1:nrow(h5_MetaNames)) {
    target <- h5_MetaNames$full_name[i]
    h5_MetaVals <- h5read(out_arrow, target)
    h5_MetaVals <- h5_MetaVals[h5_keepLogic]
    h5delete(out_arrow, target)
    h5write(h5_MetaVals, out_arrow, target)
  }
}
...
barcode_conditions <- list(
  ...
  "doublets" = meta$altius_frac > cutoffs$altius_frac & meta$peaks_frac > cutoffs$peaks_frac & meta$tss_frac > cutoffs$tss_frac & !meta$singlet,
  "singlets" = meta$altius_frac > cutoffs$altius_frac & meta$peaks_frac > cutoffs$peaks_frac & meta$tss_frac > cutoffs$tss_frac & meta$singlet
)
...
if(file.exists(in_sum)) {
  ...
  metrics_summary$total_usable_fragments <- floor(metrics_summary$ATAC.Sequenced.read.pairs * metrics_summary$ATAC.Confidently.mapped.read.pairs * metrics_summary$ATAC.Fraction.of.high.quality.fragments.in.cells)
  ...
  fragment_qc_stats <- data.frame(metric = c("Sequenced Fragments",
                                  ...
                                  count = c(metrics_summary$ATAC.Sequenced.read.pairs,
                                            #ceiling(metrics_summary$ATAC.Sequenced.read.pairs * metrics_summary$frac_waste_mitochondrial),
...
if(file.exists(in_sum)) {
  ...
  total_metrics <- data.frame(total_reads = metrics_summary$ATAC.Sequenced.read.pairs,
